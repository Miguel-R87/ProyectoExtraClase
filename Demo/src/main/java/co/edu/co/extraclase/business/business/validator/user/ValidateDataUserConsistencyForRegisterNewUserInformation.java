...
public final class ValidateDataUserConsistencyForRegisterNewUserInformation implements Validator {
	...
	protected void validateDataFormat(final UserDomain data) {
		StringFormatValueIsValidRule.executRule(data.getFirstName(), "primer nombre", "", true);
		StringFormatValueIsValidRule.executRule(data.getLastName(), "apellido","", true);
		StringFormatValueIsValidRule.executRule(data.getUsername(), "nombre de usuario","",  true);
